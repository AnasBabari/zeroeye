...
def validate_platform_config_at_startup() -> None:
    """Validate platform config using tools.config_validator before building."""
    validator = ROOT / "tools" / "config_validator.py"
    config = ROOT / "config" / "platform.json"
    if not validator.exists() or not config.exists():
        return
    try:
        result = subprocess.run(
        ...
        print(f"WARNING: could not run config validator: {exc}", file=sys.stderr)
        return
