...
def validate_platform_config_at_startup() -> None:
    """Validate platform configuration at startup using the versioned JSON Schema."""
    config_path = ROOT / "config" / "platform.json"
    if not config_path.exists():
        print("WARNING: platform config not found, skipping validation", file=sys.stderr)
        return
    try:
        from tools.config_validator import main as config_validator_main
    except ImportError:
        sys.path.insert(0, str(ROOT / "tools"))
        from config_validator import main as config_validator_main
    rc = config_validator_main([str(config_path)])
    if rc != 0:
        print("ERROR: platform configuration validation failed; aborting build", file=sys.stderr)
        sys.exit(rc)
